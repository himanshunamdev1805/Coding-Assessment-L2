...
function filterProducts(category) {
    ...
    if (selectedCategory) {
        selectedCategory.category_products.forEach(product => {
            const truncatedTitle = product.title.length > 12 ? product.title.slice(0, 12) + '...' : product.title;

            const productCard = document.createElement('div');
            productCard.classList.add('product-card');

            // Calculate percentage off if compare_at_price exists
            let percentageOff = 0;
            if (product.compare_at_price) {
                const compareAtPrice = parseFloat(product.compare_at_price);
                const price = parseFloat(product.price);
                percentageOff = Math.round(((compareAtPrice - price) / compareAtPrice) * 100);
            }
            ...
            productCard.innerHTML = `
                ...
                <p><b><span>${truncatedTitle}</span></b> • <span>${product.vendor}</span></p>
                <p><b>Rs ${product.price}</b> ${product.compare_at_price ? `<s>₹${product.compare_at_price}</s> <span style="color: #FF3737;">${percentageOff}% off</span>` : ''}</p>
    ...
    document.querySelector(`.tabs button[onclick="filterProducts('${category}')"]`).classList.add('active');
}
